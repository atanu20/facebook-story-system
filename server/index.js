...
router.post("/register", async (req,res)=>{
    ...
    try{
        ...
        let targetFile = req.files.postimg;
        ...
        // console.log(targetFile)
            ...
            let uploadDir = path.join(__dirname,'public','images','profile', num+"_"+targetFile.name);
            // let uploadDir = 'http://localhost:4001/post' + num+"_"+targetFile.name
            ...
            if(targetFile.size > 4000000 ){
               
               return res.json({submit:false,msg:"File should be less then 4 MB "})
            }
            targetFile.mv(uploadDir, async (err) => {
                try{
                    const imgname=num+"_"+targetFile.name;

                    const password=req.body.password;
                    const hash=await bcrypt.hash(password, saltRounds);
                    
                        const usedet=new userDetailsTable({name:req.body.name,email:req.body.email,profilePicture:imgname,password:hash})
                        // console.log(usedet)
...
router.post("/uploadpost",async (req,res)=>{
    ...
    // console.log(targetFile)
        ...
        const num= random.int(10000,999999);
        let uploadDir = path.join(__dirname,'public','images','post', num+"_"+targetFile.name);
        // let uploadDir = 'http://localhost:4001/post' + num+"_"+targetFile.name
    ...
    const exist=await userDetailsTable.findById(user_id)
                ...
                const usedet=new userPostTable({postimg:imgname,user:exist.name, userproimg:exist.profilePicture, user_id:req.body.user_id})
